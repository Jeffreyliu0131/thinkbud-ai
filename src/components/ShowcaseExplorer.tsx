...
import { ArrowUpRight, BookOpen, GitBranch, RotateCcw } from 'lucide-react'
...
export function SubjectExplorer() {
  ...
  return <section id="subject-explorer" tabIndex={-1} className="sx-chapter">
    ...
    <div className="sx-workspace">
      <div className="sx-topic-list" role="group" aria-label={c('代表题型', 'Representative tasks')}>{topics.map((item, index) => <button key={item.id} aria-pressed={item.id === topic.id} onClick={() => { setTopicId(item.id); setSignal('stuck') }}><span>{String(index + 1).padStart(2, '0')}</span><strong>{txt(item.title)}</strong></button>)}</div>
    ...
    <details className="sx-source-notes"><summary>{c('本节依据 · 展开查看', 'Sources for this section')}</summary><div className="sx-sources"><Source path={`functions/_shared/prompt/subjects/${subject}.ts`}>{c('学科策略源码', 'Subject policy')}</Source><Source path="functions/_shared/prompt/grade-adapters.ts">{c('学段适配源码', 'Age-band adapters')}</Source><Source path="functions/_shared/prompt/core.ts">{c('回合与情绪规则', 'Turn and emotion rules')}</Source></div></details>
  </section>
}
...
export function KnowledgeExplorer() {
  ...
  return <section id="knowledge-explorer" tabIndex={-1} className="sx-chapter">
    ...
    <div className="sx-traces">
      ...
      ].map(([zh, en, body, bodyEn]) => <li key={en}><strong>{c(zh, en)}</strong><p>{c(body, bodyEn)}</p></li>)}</ol></div>
    ...
    <details className="sx-source-notes"><summary>{c('本节依据 · 展开查看', 'Sources for this section')}</summary><div className="sx-sources"><Source path="src/components/ThinkingTree.tsx">{c('思考链组件', 'Thinking-trace component')}</Source><Source path="src/lib/knowledgeGraph.ts">{c('知识上下文构建', 'Knowledge-context builder')}</Source><Source path="shared/kcVocabulary.ts">{c('知识点词表', 'Concept vocabulary')}</Source><Source path="functions/api/extract-knowledge.ts">{c('提取与持久化', 'Extraction and persistence')}</Source><Source path="shared/bkt.ts">{c('BKT 更新公式', 'BKT update function')}</Source></div></details>
  </section>
}

export function SystemExplorer({ ragDemo }: { ragDemo: React.ReactNode }) {
  const c = useCopy()
  return <>
    <section id="system-explorer" tabIndex={-1} className="sx-chapter">
      ...
      <details className="sx-source-notes"><summary>{c('本节依据 · 展开查看', 'Sources for this section')}</summary><div className="sx-sources"><Source path="functions/_shared/prompt/index.ts">{c('Prompt 组装', 'Prompt assembly')}</Source><Source path="docs/TEXTBOOK_RAG.md">{c('RAG 合同与边界', 'RAG contract and limits')}</Source><Source path="docs/ARCHITECTURE.md">{c('完整架构', 'Architecture')}</Source></div></details>
    </section>
    <section id="product-decisions" tabIndex={-1} className="sx-chapter">
      ...
      <details className="sx-source-notes"><summary>{c('本节依据 · 展开查看', 'Sources for this section')}</summary><div className="sx-sources"><Source path="docs/CASE_STUDY.md">{c('产品决策记录', 'Product decisions')}</Source><Source path="docs/FIELD_PILOT_PROTOCOL.md">{c('后续观察方案', 'Future observation protocol')}</Source><a className="sx-source" href="https://github.com/Jeffreyliu0131/thinkbud-ai" target="_blank" rel="noreferrer"><GitBranch size={14} aria-hidden="true" />{c('查看完整项目', 'Explore the repository')}</a></div></details>
    </section>
  </>
}
...
export function SubjectPreview() {
  ...
  return <div className="tb-hero__preview"><article className="tb-lesson sx-preview" aria-label={c('三科引导预览', 'Three-subject coaching preview')}>
    ...
    <div className="tb-lesson__footer"><span>{c('不同学科，共同保留思考空间', 'Different subjects. Room to think.')}</span></div>
