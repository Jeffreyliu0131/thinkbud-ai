...
function AnswerForm({ task, stage, step, paused, onAnswer }: {
  ...
  const input = (index: number, name: string) => <input key={index} aria-label={t(name)} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} name={`answer${index}`} type="text" inputMode="numeric" autoComplete="off" maxLength={5} autoFocus={index === 0} required />
...
export default function PracticePage() {
  ...
  useEffect(() => {
    if (phase && ['transfer-ready', 'waiting', 'complete'].includes(phase)) cardRef.current?.focus()
  }, [phase])
  ...
  return <main data-language={locale} className={`practice-page ${session ? 'practice-page--active' : ''}`}><nav className="practice-nav" aria-label={t("练习导航")}><a href={demoRoute('/showcase')}><span className="practice-brand__mark" aria-hidden="true"><BudMascot /></span><strong>ThinkBud</strong></a>
    ...
    {!session || !state ? <section className="practice-start"><div><span className="practice-eyebrow">{t("一条完整的练习路径")}</span><h2>{t("学过，不急着算“掌握”。")}</h2>
