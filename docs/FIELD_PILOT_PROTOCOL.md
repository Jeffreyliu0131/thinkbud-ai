...
## Implemented prototype for the next review · 2026-09-07

Use `/practice` to compare the selectable question-first and different-example-after-struggle policies on the provided synthetic distributive-property items. Assistance shown by the interface is recorded. Independent checks require both expansion and calculation, and distinguish retries from first completions. Asking for help preserves the unfinished check before returning to a fresh practice pair.

The current teaching text is preset, not generated by a live model. Teacher usefulness, age fit, frustration recovery, and the educational value of the chosen item sequence remain human-review questions. Future comparison should use matched tasks and counterbalanced policy order; this implementation does not claim either policy wins.

The delayed-flow preview is for reviewers who cannot wait 24 hours. It uses a different item, leaves the formal review pending, and is excluded from exports. Passing the synthetic controlled-clock eval proves the timer/state boundary, not delayed retention. Optional local progress and exports are adult-preview observations and must not be relabeled as a child study, validated mastery, or Career Evidence.
