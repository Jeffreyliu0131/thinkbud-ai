...
页面默认跟随浏览器语言，也可在页头选择 **中文 / English / 跟随系统**。顺着页面阅读到“数学实操”，点击 **展开数学练习 / Open maths exercise**，在本节内体验。原有独立练习链接仍可直接使用，左上角提供“返回项目介绍”。确认成人体验，保留默认“连续两次困难后，换例子解释”。需要演示刷新恢复时勾选本机保存；重新演示使用页面的“清除本次记录并重新开始”。以下步骤只验证预设的浏览器交互，不是登录、语音或真实 AI 服务的使用教程。
...
5. **追到 AI、RAG 与实现。** 查看上下文和输出检查的分工，在当前章节展开合成检索示例。故障和无结果不展示引用；最后从实现地图进入对应源码，核对页面运行、服务端代码、适配层及待验证事项。

2026-09-22 阅读路径修订：模块按顺序阅读，不再提供跨章节跳转菜单。数学练习与检索示例在所属章节原地展开；收起后可继续阅读，重开练习保留当前页面输入和进度。页面中英文切换保留当前题型和学习者状态；英语学科原始辅导仍是中文解释，英文 UI 只是展示译文。
...
5. **追问时再看技术证据。** 收起练习继续阅读，到 AI/RAG 章节展开检索示例，切换预设故障/无结果，观察引用消失。需要报告时，再展开页面末尾的“查看实现与证据”。合成报告验证指定规则，不证明实时模型质量、三科互动覆盖或学习效果。
