...
ThinkBud is an AI learning and thinking coach prototype for primary-school **Chinese, maths, and English**. Its product goal is to help learners work through homework and explain their reasoning through guided dialogue. The linked page explores **24 authored task examples** across Chinese, maths and English, the two implemented age bands, thinking traces, knowledge observations, and RAG. A separate answer-entry workflow covers **grade-4 maths practice**, from guided steps to a new problem without in-app hints.
...
Open the printed URL ending in `/thinkbud-ai/`, then scroll to the maths workflow chapter and select **展开数学练习 / Open maths exercise**. It opens within that chapter; collapse and reopen preserve the current in-memory exercise and unsubmitted input. The practice deep link is `/thinkbud-ai/#/practice`; the product overview is `/thinkbud-ai/#/showcase`. Questions and coaching are preset, with no live AI, account, SMS, OCR, voice or business database.
...
Local validation: `npm run verify` passed (454 tests passed, 2 existing skips), including lint, TypeScript, public-boundary checks, the three synthetic gates and service build. Static demo build, package boundary and evidence consistency checks passed separately. In-app browser checks covered three-subject previews, task and age-band switching, Chinese writing and English grammar responses, locale preservation, BKT neutral/update/reset, RAG fault citation removal, anchored/keyboard navigation, the preserved maths practice entry, and English layouts at 390px and 320px without page-level horizontal overflow. [Reviewed captures](docs/showcase/README.md#expanded-three-subject-showcase--2026-09-22). The owner authorised commit, push and publication of this reviewed candidate on 2026-09-22. The [commit-specific CI](https://github.com/Jeffreyliu0131/thinkbud-ai/actions/runs/35690247044) and [Pages deployment](https://github.com/Jeffreyliu0131/thinkbud-ai/actions/runs/35690373396) both passed. The live `build-info.json` and three 200 JSON reports identify application source `3c2e9196493309e4bdcd12b90a7595ddf6c1ae36` with `sourceDirty=false`; live demo-input SHA-256 is `d1727f2aaa392846de925f38b9a80e255096109424b8dc0d54c4701794f544c2`. Online browser checks confirmed the new three-subject entry, Chinese composition preview, English grammar reasoning, the RAG handoff and fault-state citation removal, with no captured browser errors or warnings. Local screenshots retain their original pre-publication provenance. This documentation-only follow-up does not change deployed application code.


## Reading and interaction repair · 2026-09-22

**Published to the existing [ThinkBud walkthrough](https://jeffreyliu0131.github.io/thinkbud-ai/).** First-time reading now follows one continuous sequence: product introduction → subject reasoning → optional inline maths exercise → knowledge observations → AI/RAG → decisions and evidence. The first screen has no section-jump links or exercise-route CTA; its subject controls only change the adjacent preview. The sticky jump menu and repeated entry cards are removed.

The maths exercise mounts only when opened in its own chapter, remains mounted while collapsed, and keeps the current input, practice events and all surrounding subject/knowledge selections. Closing restores focus to its named entry; it scrolls back there only if that entry is outside the viewport. Opening performs no route or scroll operation. Step changes focus the answer field without scrolling in embedded mode. The retained standalone `/practice` link has a visible upper-left **Back to project introduction** control and an explicit page label; it is no longer part of the main reading path.

The RAG state explorer expands beside its RAG explanation instead of jumping into another chapter. Source references and technical reports remain available as optional local disclosures. No teaching policy, BKT update, delayed-check gate, persistence/export rule or service boundary is changed.

Local acceptance passed: lint, TypeScript, full `verify` (455 tests passed, 2 existing skips), the focused retrieval-disclosure regression after its label change, the static package boundary and evidence consistency. Browser checks exercised first opening without viewport movement, typed-input recovery after collapse, keyboard focus after submission, 390px/320px English layouts, direct-link return, and RAG open/state/collapse within its own chapter. [Reading-flow captures](docs/showcase/README.md#continuous-reading-and-inline-practice--2026-09-22). The correction continues the authorised website update. [CI](https://github.com/Jeffreyliu0131/thinkbud-ai/actions/runs/35692440487) and [Pages publication](https://github.com/Jeffreyliu0131/thinkbud-ai/actions/runs/35692569772) passed for application source `c0286f181a5c3b806ee330bcd989edd0ab1a4a33`. The live build stamp and all three JSON reports return 200 and identify that exact commit with `sourceDirty=false`; live demo-input SHA-256 is `864080dea42384695e0df963ef5e24c39307dbddd4480c70d7d838d1fe483396`. Online native-click verification kept scrollY at 2160 when opening practice, preserved the typed 5 across collapse/resume without changing the URL, and confirmed the RAG disclosure remains in its chapter and removes citations on failure. No browser errors or warnings were captured. This follow-up records publication without changing application code.
