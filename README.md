...
**Local revision in progress; the earlier expanded release remains online until publication is verified.** First-time reading now follows one continuous sequence: product introduction → subject reasoning → optional inline maths exercise → knowledge observations → AI/RAG → decisions and evidence. The first screen has no section-jump links or exercise-route CTA; its subject controls only change the adjacent preview. The sticky jump menu and repeated entry cards are removed.
